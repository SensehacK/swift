...
extension TMDBClient {
    ...
    */
    func authenticateWithViewController(_ hostViewController: UIViewController, completionHandlerForAuth: @escaping (_ success: Bool, _ errorString: String?) -> Void) {
        ...
        getRequestToken() { (success, requestToken, errorString) in
            ...
            if success {
                ...
                print("In Authenticate View Controller")
                print(requestToken)
                    ...
                    if success {
                        self.getSessionID(requestToken) { (success, sessionID, errorString) in
                            ...
                            if success {
                                ...
                                //Prints for easy code deciphering
                                print("SessionID clause in func AuthenicateViewController")
                                print(sessionID)
                                self.getUserID() { (success, userID, errorString) in
                                    ...
                                    if success {
                                        ...
                                        if let userID = userID {
                                            ...
                                            //Prints for easy code deciphering
                                            print("userID clause in func AuthenicateViewController")
                                            print(userID)
    ...
    private func getRequestToken(_ completionHandlerForToken: @escaping (_ success: Bool, _ requestToken: String?, _ errorString: String?) -> Void) {
         ...
         print("In getRequestToken View Controller")
       ...
       let _ = taskForGETMethod(Methods.AuthenticationTokenNew, parameters: parameters)  { (results , error) in
            ...
            if let error = error {
                ...
                if let requestToken = results?[TMDBClient.JSONResponseKeys.RequestToken] as? String {
                    ...
                    print("Couldnt find key in \(results)")
                    completionHandlerForToken(false, nil, "login Failed (Request key token")
                }
            }
   ...
   private func getSessionID(_ requestToken: String?, completionHandlerForSession: @escaping (_ success: Bool, _ sessionID: String?, _ errorString: String?) -> Void) {
    ...
    let _ = taskForGETMethod(Methods.AuthenticationSessionNew, parameters: parameters as [String : AnyObject]) {
        (results , error) in
        if let error = error
        {
            print(error)
            ...
            if let sessionID = results?[TMDBClient.JSONResponseKeys.SessionID] as? String {
                ...
                print("Couldnt find it \(TMDBClient.JSONResponseKeys.SessionID) in \(results)")
                completionHandlerForSession(false, nil, "Couldnt find Key Session ID")
            }
        }
    ...
    private func getUserID(_ completionHandlerForUserID: @escaping (_ success: Bool, _ userID: Int?, _ errorString: String?) -> Void) {
        ...
        */
            if let error = error {
                print(error)
                completionHandlerForUserID(false, nil, "Error Logged in User ID")
            } else
            {
                ...
                {
                    print("Couldnt find the user ID  \(TMDBClient.JSONResponseKeys.UserID) in \(results)")
                        completionHandlerForUserID(false, nil, "Error Couldnt find User ID in UserID")
                }
